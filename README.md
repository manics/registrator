...
If `-iponly` is passed all ports will be ignored, and a single internal ip (with port 0) will be published for each container. This is useful when running containers on a bridge with externally accessible IPs since ports do not need to be mapped.
